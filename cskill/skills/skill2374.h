//Skill Parser (c)from elementskill.dll v1.7.4

/* SKILL DESCRIPTION BEGIN */
/*
23740  "Î¦Dragon Rising"23741  "^ffff00Î¦Dragon Rising

^ffcb4aRangeã€€^ffffffMelee
^ffcb4aManaã€€^96f5ff140^ffffff
^ffcb4aChannelã€€^ffffff0.4ã€€^ffcb4aseconds
^ffcb4aCastã€€^ffffff1.5ã€€^ffcb4aseconds
^ffcb4aCooldownã€€^ffffff15.0ã€€^ffcb4aseconds
^ffcb4aWeaponã€€^ffffffMelee weapons

^ffcb4aRequired Cultivationã€€^ffffffAware of the Myriad
Swing your weapon in a rising attack, dealing
base physical damage plus ^96f5ff105%%^ffffff of weapon damage
plus ^96f5ff5834^ffffff and stunning the target for ^96f5ff4^ffffff seconds.
Never misses

Axes, Poleaxes, Hammers and Polearms stun the target for ^96f5ff7^ffffff seconds.
Poleblades, Pikes and Clubs increases the range to ^96f5ff20^ffffff meters plus ^96f5ff100%%^ffffff of weapon damage
Blades and Swords reduces the channeling time to ^96f5ff0.2^ffffff seconds
and casting time to ^96f5ff1.4^ffffff seconds plus ^96f5ff300%%^ffffff of weapon damage.

^a6caf0Costs ^ffffff1^a6caf0 Spark.

^f7dfa5Sage version has a 35%% chance to consume no Sparks.^96f5ff^f7dfa5

"*/
/* SKILL DESCRIPTION END */

#ifndef __CPPGEN_GNET_SKILL2374
#define __CPPGEN_GNET_SKILL2374
namespace GNET
{
#ifdef _SKILL_SERVER
    class Skill2374:public Skill
    {
        public:
        enum
        { SKILL_ID = 2374 };
        Skill2374 ():Skill (SKILL_ID)
        {
        }
    };
#endif
    class Skill2374Stub:public SkillStub
    {
        public:
#ifdef _SKILL_SERVER
        class State1:public SkillStub::State
        {
            public:
            int GetTime (Skill * skill) const
            {
                return skill->GetPlayer()->GetCurweapon() != 1 ? 400 : 200;
            }
            bool Quit (Skill * skill) const
            {
                return false;
            }
            bool Loop (Skill * skill) const
            {
                return false;
            }
            bool Bypass (Skill * skill) const
            {
                return false;
            }
            void Calculate (Skill * skill) const
            {
                skill->GetPlayer ()->SetDecmp (28);
                skill->GetPlayer ()->SetPray (1);
            }
            bool Interrupt (Skill * skill) const
            {
                return false;
            }
            bool Cancel (Skill * skill) const
            {
                return 1;
            }
            bool Skip (Skill * skill) const
            {
                return 0;
            }
        };
#endif
#ifdef _SKILL_SERVER
        class State2:public SkillStub::State
        {
            public:
            int GetTime (Skill * skill) const
            {
                return skill->GetPlayer()->GetCurweapon() != 1 ? 1500 : 1400;
            }
            bool Quit (Skill * skill) const
            {
                return false;
            }
            bool Loop (Skill * skill) const
            {
                return false;
            }
            bool Bypass (Skill * skill) const
            {
                return false;
            }
            void Calculate (Skill * skill) const
            {
                skill->GetPlayer ()->SetDecmp (112);
                skill->SetPlus (5834);
                skill->SetRatio (skill->GetPlayer()->GetCurweapon() == 1 ? 4.05 : skill->GetPlayer()->GetCurweapon() == 5 ? 2.05 : 1.05);
                skill->SetDamage (skill->GetAttack ());
                skill->SetDamage (skill->GetAttack ());
                skill->GetPlayer ()->SetPerform (1);
            }
            bool Interrupt (Skill * skill) const
            {
                return false;
            }
            bool Cancel (Skill * skill) const
            {
                return 0;
            }
            bool Skip (Skill * skill) const
            {
                return 0;
            }
        };
#endif
#ifdef _SKILL_SERVER
        class State3:public SkillStub::State
        {
            public:
            int GetTime (Skill * skill) const
            {
                return 0;
            }
            bool Quit (Skill * skill) const
            {
                return false;
            }
            bool Loop (Skill * skill) const
            {
                return false;
            }
            bool Bypass (Skill * skill) const
            {
                return false;
            }
            void Calculate (Skill * skill) const
            {
            }
            bool Interrupt (Skill * skill) const
            {
                return false;
            }
            bool Cancel (Skill * skill) const
            {
                return 0;
            }
            bool Skip (Skill * skill) const
            {
                return 0;
            }
        };
#endif
        Skill2374Stub ():SkillStub (2374)
        {
            cls                 = 0;
#ifdef _SKILL_CLIENT
            name                = L"Ðþ¡¤ÉýÁúÆÆ";
            nativename          = "Ðþ¡¤ÉýÁúÆÆ";
            icon                = "Ðþ¡¤ÉýÁúÆÆ.DDS";
#endif
            max_level           = 1;
            type                = 1;
            apcost              = 100;
            arrowcost           = 0;
            apgain              = 0;
            attr                = 7;
            rank                = 20;
            eventflag           = 0;
            is_senior           = 1;
            posdouble           = 0;
            clslimit            = 0;
            time_type           = 0;
            showorder           = 1117;
            allow_land          = 1;
            allow_air           = 1;
            allow_water         = 1;
            allow_ride          = 0;
            auto_attack         = 0;
            long_range          = 0;
            restrict_corpse     = 0;
            allow_forms         = 1;
            {
                restrict_weapons.push_back (9);
            }
            {
                restrict_weapons.push_back (1);
            }
            {
                restrict_weapons.push_back (5);
            }
#ifdef _SKILL_CLIENT
            effect              = "ÐÂ_°ÔÍõÁú·É.sgc";
#endif
            range.type          = 0;
            doenchant           = true;
            dobless             = true;
            commoncooldown      = 0;
            commoncooldowntime  = 0;
            pre_skills.push_back (std::pair < ID, int >(406, 1));
            pre_skills.push_back (std::pair < ID, int >(414, 1));
            pre_skills.push_back (std::pair < ID, int >(398, 1));
#ifdef _SKILL_CLIENT
            aerial_effect       = "";
            gfxfilename         = "";
            gfxhangpoint        = "0";
#endif
#ifdef _SKILL_SERVER
            statestub.push_back (new State1 ());
            statestub.push_back (new State2 ());
            statestub.push_back (new State3 ());
#endif
        }
        virtual ~ Skill2374Stub ()
        {
        }
        float GetMpcost (Skill * skill) const
        {
            return (float) (140);
        }
        int GetExecutetime (Skill * skill) const
        {
            return 1500;
        }
        int GetCoolingtime (Skill * skill) const
        {
            return 15000;
        }
        int GetRequiredLevel (Skill * skill) const
        {
            static int aarray[10] = { 100 };
            return aarray[skill->GetLevel () - 1];
        }
        int GetRequiredSp (Skill * skill) const
        {
            static int aarray[10] = { 1000000 };
            return aarray[skill->GetLevel () - 1];
        }
        int GetRequiredMoney (Skill * skill) const
        {
            static int aarray[10] = { 1000000,1000000,1000000,1000000,1000000,1000000,1000000,1000000,1000000,1000000 };
            return aarray[skill->GetLevel () - 1];
        }
        int GetRequiredItem (Skill * skill) const
        {
            static int aarray[10] = { 42135,42135,42135,42135,42135,42135,42135,42135,42135,42135 };
            return aarray[skill->GetLevel () - 1];
        }
        float GetRadius (Skill * skill) const
        {
            return (float) (0);
        }
        float GetAttackdistance (Skill * skill) const
        {
            return (float) (0);
        }
        float GetAngle (Skill * skill) const
        {
            return (float) (1 - 0.0111111 * (0));
        }
        float GetPraydistance (Skill * skill) const
        {
            //return (float) (skill->GetPlayer ()->GetRange ());
            return(float) (skill->GetPlayer()->GetCurWeapon() == 5 ? 20 : skill->GetPlayer()->GetRange());
        }
#ifdef _SKILL_CLIENT
        int GetIntroduction (Skill * skill, wchar_t * buffer, int length, wchar_t * format) const
        {
            return _snwprintf (buffer, length, format);
        }
#endif
#ifdef _SKILL_SERVER
        int GetEnmity (Skill * skill) const
        {
            return 3000;
        }
#endif
#ifdef _SKILL_SERVER
        bool StateAttack (Skill * skill) const
        {
            skill->GetVictim ()->SetProbability (1.0 * skill->GetPerformerCurWeapon() != 1 ? 100 : 0);
            skill->GetVictim ()->SetTime (4500 + (skill->GetPerformerCurWeapon () == 9 ? 2000 : 0));
            skill->GetVictim ()->SetValue (2);
            skill->GetVictim ()->SetPalsy (1); //stun
            
            /*skill->GetVictim ()->SetProbability (1.0 * skill->GetT2 ()); //espada
            skill->GetVictim ()->SetTime (11000);
            skill->GetVictim ()->SetRatio (skill->GetPlayer ()->GetCls () == -1 ? 0.5 : 1);
            skill->GetVictim ()->SetIncrange (20);*/
            
            /*skill->GetVictim ()->SetProbability (1.0 * skill->GetT1 () > 0 ? 100 : 0); // lan?a
            skill->GetVictim ()->SetTime (6000);
            skill->GetVictim ()->SetAmount (skill->GetT1 ());
            skill->GetVictim ()->SetBleeding (1);*/
            return true;
        }
#endif
#ifdef _SKILL_SERVER
        bool BlessMe (Skill * skill) const
        {
            skill->GetVictim ()->SetProbability (1.0 * 35);
            skill->GetVictim ()->SetTime (1500);
            skill->GetVictim ()->SetShowicon (0);
            skill->GetVictim ()->SetAmount (100);
            skill->GetVictim ()->SetValue (0);
            skill->GetVictim ()->SetApgencont2 (1);
            return true;
        }
#endif
#ifdef _SKILL_SERVER
        float GetTalent0 (PlayerWrapper * player) const
        { //machado
        return (float) (player->GetCurweapon () == 9 ? 2000 : 0);
    }
#endif
#ifdef _SKILL_SERVER
    float GetTalent1 (PlayerWrapper * player) const
    { //lan?a
    return (float) (player->GetCurweapon () == 5 ? 2 * player->GetEquipattack () : 0);
}
#endif
#ifdef _SKILL_SERVER
float GetTalent2 (PlayerWrapper * player) const
{ //espada
return (float) (player->GetCurweapon () == 1 ? 100 : 0);
}
#endif
#ifdef _SKILL_SERVER
bool TakeEffect (Skill * skill) const
{
    return true;
}
#endif
#ifdef _SKILL_SERVER
float GetEffectdistance (Skill * skill) const
{
    return (float) (19.7);
}
#endif
#ifdef _SKILL_SERVER
int GetAttackspeed (Skill * skill) const
{
    return 13;
}
#endif
#ifdef _SKILL_SERVER
float GetHitrate (Skill * skill) const
{
    return (float) (1.7);
}
#endif
};
}
#endif