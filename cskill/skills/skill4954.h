...
/*
49540  "Î”Fata Morgana"49541  "^ff0000Î”Fata Morganaã€€ã€€ã€€ã€€ã€€ã€€ã€€Lv%d|if&dynskilllv()>0&^ffcb4a +%d&|
...
Summons a shadow that tracks down the enemy for ^96f5ff|if&(skilllv()+dynskilllv())<2&10&20|^ffffff seconds,
while increasing your Attack Level by ^96f5ff%d^ffffff.
If the tracked enemy is within the attack range, the tracked target will also be attacked when you attack other targets with skills.
If the attacked target and the
tracked target are both within the attack range of yourself and your shadow,
they will both take twice the damage.

|if&(skilllv()+dynskilllv())>2&3Lv.ï¼š^ffffffWhile the Shadow is active, every ^96f5ff3^ffffff seconds, cleans
your Negative status once. &^808080Lv3:While the Shadow is active, every 3 seconds, cleans
your Negative status once."*/
...
namespace GNET
{
    ...
    class Skill4954Stub:public SkillStub
    {
        ...
        {
            skill->GetVictim ()->SetProbability (1.0 * 100); // Aumenta o NÃ­vel de Ataque
            skill->GetVictim ()->SetTime (11000*skill->GetLevel());
            ...
            skill->GetVictim ()->SetTime (11000*skill->GetLevel());
            skill->GetVictim ()->SetPragaBless(1);
            return true;
        }
#endif
#ifdef _SKILL_SERVER
