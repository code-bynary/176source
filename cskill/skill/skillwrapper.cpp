...
int SkillWrapper::StartSkill( SKILL::Data & skilldata, object_interface player, const A3DVECTOR& target_pos,const XID * target, int size, int & next_interval)
{
	...
	if(skill->GetEventFlag() == EVENT_NEW_ARMOR)
	{
		...
		if(!player.TestCoolDown(skilldata.id+COOLINGID_BEGIN)) 
			return -1;
		...
		if(skill->GetType()!=TYPE_JUMP) return -1;
		
		// For�a o Salto
		float range = skill->GetPraydistance();
		float jump_plus = range;
		player.ShortJump(jump_plus,true);
...
int SkillWrapper::Run( SKILL::Data & skilldata, object_interface player, const XID * target, int size , int& next_interval)
{
	...
	else
	{
		if(skill->IsStateEnd() && SkillStub::IsComboPreskill(skilldata.id))
		{
			//printf("Run isStateEnd: id = %d , ret = %d \n", skilldata.id , ret );			
			OnComboPreSkillEnd(skill,player);
		}
	}
...
int SkillWrapper::CastRune(SKILL::Data & skilldata, object_interface player, int level)
{
	...
	skill->SetPlayerHighestLevel(w_player.GetHighestLevel());
...
void SkillWrapper::Store( archive & ar )
{
	...
	size = MultiCd_map.size();
	ar << size;
	for( MultiCooldownMap::const_iterator it = MultiCd_map.begin(); it != MultiCd_map.end(); ++it )
	{
		ar << (*it).first;
		ar << (*it).second.first_use_time;
		ar << (*it).second.count;		
	}
...
int SkillWrapper::ActivateSkill(object_interface player, ID id, int level, int type)
{
	...
	else if (type)
	{
		...
		as[1].id = id;
		as[1].level = level;
		player.AddFilter(new filter_Activateskill2(player, id, 0));
		}
	}
	else
	{
		as[0].id = id;
		as[0].level = level;
		player.AddFilter(new filter_Activateskill(player, id, 0));
	}
	return 0;
}
...
int SkillWrapper::ActivateReboundSkill(object_interface player, ID id, int level, int trigger_prob, int period)
{
	...
	as[2].id = id;
	as[2].level = level;
	player.AddFilter(new filter_Activatereboundskill(player, trigger_prob, period, id, 60));
	return 0;
}
...
void SkillWrapper::Swap(SkillWrapper& sw)
{
	...
	sw.MultiCd_map.swap(MultiCd_map);
	/*for(unsigned int i = 0; i < 3; i ++)
	{
...
int SkillWrapper::SetSealedforTW(object_interface player, int second)
{
	player.AddFilter(new filter_SealedforTW(player,second,0));
	return 0;
}
...
void SkillWrapper::SetSkillTalent(Skill* skill, const int* list)
{
	for(int i=0;i<5;i++)
	{
		...
		//{
			...
			//printf("SetSkillTalent: skill id =  %d, list[%d] = %d \n", skill->GetId(), i, list[i]);
		//}	
	}		
}
...
/*void SkillWrapper::GetMultiCooldownSkills()
{
	auto skillsmap = [&](const StorageMap& skillsMap)
	{
		for (const auto& pair : skillsMap)
        {
			const SkillStub* stub = SkillStub::GetStub(pair.first);
			if(stub)
			{
				if(stub->GetCoolDownLimit(pair.first) > 0)
				{
					MultiCooldown data;
					data.first_use_time = static_cast<int>(time(NULL));
					data.count = stub->GetCoolDownLimit(pair.first);
					MultiCd_map[pair.first] = data;
				}
			}
		}
	}
...
void SkillWrapper::ModifyMulticooldown(ID id, int count)
{
	if(count >= 0)
	{
		MultiCd_map[id].count = count;
	}
	else
	{
		MultiCd_map.erase(id);
	}
}

int SkillWrapper::GetMultiCooldownTime(ID id)
{
	MultiCooldownMap::iterator iter = MultiCd_map.find(id);
	if(iter != MultiCd_map.end())
	{
		return iter->second.first_use_time;
	}
	else
	{
		int currentTime = static_cast<int>(time(NULL));
		MultiCd_map[id].first_use_time = currentTime;
		int count = GetCoolDownLimit(id);
		MultiCd_map[id].count = count;
		return currentTime;
	}
}
void SkillWrapper::OnComboPreSkillEnd(Skill* skill, object_interface player)
{
	...
	//printf("OnComboPreSkillEnd: id = %d \n", skill->GetId());		
	if(CheckComboBreak(now,true))
	{
		//printf("OnComboPreSkillEnd inside: id = %d \n", skill->GetId());	
...
bool SkillWrapper::ConvertAndUpdateBalls(int mode, int& new_vstate, int& old_vstate, int& hstate)
{
    old_vstate = black_white_ball.vstate;
    black_white_ball.ConvertSpecificBalls(mode);
    new_vstate = black_white_ball.UpdateVstate(old_vstate);
    hstate = black_white_ball.balls;

    return true; // Supondo que a conversao sempre seja bem-sucedida.
}
...
/*
void SkillWrapper::ResetSkillFeatureCooldown(object_interface player, int feature)
{
    for (StorageMap::const_iterator it = map.begin(); it != map.end(); ++it)
    {
        const SkillStub * stub = SkillStub::GetStub(it->first);

        if (stub && stub->feature == feature)
        {
            int cooldownId = it->first + COOLINGID_BEGIN;

            player.ResetCoolDown(cooldownId);
        }
    }
}
*/
...
void SkillWrapper::ResetSkillFeatureCooldown(object_interface player, int feature)
{
    auto resetCooldownForMap = [&](const StorageMap& skillsMap) 
	{
        for (const auto& pair : skillsMap)
        {
            const SkillStub* stub = SkillStub::GetStub(pair.first);
            ...
            if (stub && stub->feature == feature)
            {
                int cooldownId = pair.first + COOLINGID_BEGIN;
    ...
    resetCooldownForMap(map);
    resetCooldownForMap(glyph_map);
}

void SkillWrapper::ResetSpecificSkillCooldowns(object_interface player, int feature, int id1, int id2, int id3, int id4, int id5, int id6)
{
    int skillIds[6] = {id1, id2, id3, id4, id5, id6};
    ...
    auto resetCooldownForMap = [&](const StorageMap& skillsMap) {
        for (const auto& pair : skillsMap)
        {
            ...
            if ((stub && stub->feature == feature) || std::find(std::begin(skillIds), std::end(skillIds), pair.first) != std::end(skillIds))
            {
                int cooldownId = pair.first + COOLINGID_BEGIN;
                player.ResetCoolDown(cooldownId);
            }
...
void SkillWrapper::ReduceSkillCooldowns(object_interface& player, int feature, int secondsToReduce, int excludeSkillId, int id1, int id2, int id3, int id4, int id5, int id6)
{
    int skillIds[6] = {id1, id2, id3, id4, id5, id6};

    auto reduceCooldownForMap = [&](const StorageMap& skillsMap) {
        for (const auto& pair : skillsMap)
        {
            const SkillStub* stub = SkillStub::GetStub(pair.first);

            if (pair.first != excludeSkillId && ((stub && stub->feature == feature) || std::find(std::begin(skillIds), std::end(skillIds), pair.first) != std::end(skillIds)))
            {
				int cooldownId = pair.first + COOLINGID_BEGIN;
                player.CoolDownReduceAtr(cooldownId, secondsToReduce);
            }
        }
    };

    reduceCooldownForMap(map);
    reduceCooldownForMap(glyph_map);
}


int SkillWrapper::CheckSkills()
{
	int countGod = 0;
	int countEvil = 0;
	...
	for( StorageMap::const_iterator it = map.begin(); it != map.end(); ++it )
	{
        ...
        if (stub)
        {
			if(stub->rank >= 20 && stub->rank <= 22) countGod++;
			if(stub->rank >= 30 && stub->rank <= 32) countEvil++;
		}
	}
	//printf("CheckSkills countGod = %d \n", countGod);
	//printf("CheckSkills countEvil = %d \n", countEvil);

	if(countGod > countEvil)
	{
		return 1;
	}
	if(countEvil > countGod)
	{
		return 2;
	}
	return 0;
...
void SkillWrapper::MarkOverriddenRecursively(ID skillId) {
    const SkillStub* stub = SkillStub::GetStub(skillId);
    if (!stub) return; // Exit if stub is null

    // Iterate through the prerequisite skills of the current skill
    for (const auto& preSkill : stub->pre_skills) {
        ID preSkillId = preSkill.first;
        auto it = map.find(preSkillId);
        if (it != map.end()) {
            // Mark the found prerequisite skill as overridden
            it->second.overridden = 1;
        }

        // Recursively check this prerequisite skill's own prerequisites
        MarkOverriddenRecursively(preSkillId);
    }
}
...
void SkillWrapper::DoubleCleaner(int rank) {
    for (auto it = map.begin(); it != map.end(); ++it) {
        const SkillStub* stub = SkillStub::GetStub(it->first);
        if (stub) {
            if ((rank == 2 && stub->rank >= 20 && stub->rank <= 22) ||
                (rank == 1 && stub->rank >= 30 && stub->rank <= 32)) {
                map.erase(it);
                DoubleCleaner(rank);
                return;
            } else if (stub->rank >= 40) {
                // For skills with rank >= 40, mark their prerequisites (recursively) as overridden
                MarkOverriddenRecursively(it->first);
            }
        }
    }
...
int SkillWrapper::SetRedTeamIcon(object_interface player, int second)
{
	player.AddFilter(new filter_Icon(player, second, 0, VSTATE_ARENARED, FILTER_ARENAGUANYU, filter::FILTER_MASK_UNIQUE));
	return 0;
}

int SkillWrapper::SetBlueTeamIcon(object_interface player, int second)
{
	player.AddFilter(new filter_Icon(player, second, 0, VSTATE_ARENABLUE, FILTER_ARENAGUANYU, filter::FILTER_MASK_UNIQUE));
	return 0;
}
...
int SkillWrapper::NewArmorActivateSkill(object_interface player, ID id, int level)
{
	...
	{
		if(!player.TestCoolDown(id+COOLINGID_BEGIN)) 
			return -1;
