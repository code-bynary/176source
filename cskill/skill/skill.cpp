...
bool SkillStub::ReqSecLevel(Skill * skill ) const
{
	...
	if(req >= 20 && req <= 32 && cur >= 20 && cur <= 32)
	{
		if(cur < req) 
		{
			return false;
...
int SkillStub::Condition(Skill *skill) const
{
	...
	// Nova ultimate do Macaco
	if(!skill->GetPlayer ()->GetUltimateEnabled ())
	{
	...
	if(itemcost > 0 && itemcostcount > 0 && skill->GetSection() <= 1)	
	{
		if(!skill->GetPlayer()->SetCheckitem(itemcost, itemcostcount))
			return 10;
	}
	...
	if(skill->GetPlayer ()->GetCls () != 12 || skill->GetPlayer ()->GetCls () != 11 )
	{
...
int SkillStub::ElfCondition(Skill *skill) const
{
	...
	if(itemcost > 0 )
	{
		if(!skill->GetPlayer()->SetCheckitem(itemcost, 1))
			return 10;
	}		
...
bool Skill::CanAttack(bool cost)
{
	...
	if(cost && stub->itemcost>0 && stub->itemcostcount>0 && GetSection() <= 1 )
	{
		if(!player->SetUseitem(stub->itemcost, stub->itemcostcount))
			return false;
	}
...
void Skill::SetConsumedCharge(unsigned int c)
{
	if(c > 0)
	{
		if(pdata)
		{
			//printf("Skill::SetConsumedCharge: val = %d \n", c);
			pdata->charge = c;
		}
	}
}

unsigned int Skill::GetConsumedCharge()
{
	if(pdata)
		return pdata->charge;
	if(attackmsg)
	...
	else
	{
		...
		if(casting_reduce > 0)
		{
			...
			if(casting_reduce == 1)
				...
				time = (int)(casting_reduce*0.01f*(100 - prayspeed) + 0.01f);
		}
		/*if(casting_reduce > 0)
		{
			time = casting_reduce;
...
void Skill::PrepareTalent(PlayerWrapper* player)
{
	...
	/*
	t0 = (unsigned int)round(stub->GetTalent0(player));
	t1 = (unsigned int)round(stub->GetTalent1(player));
	t2 = (unsigned int)round(stub->GetTalent2(player));
	t3 = (unsigned int)round(stub->GetTalent3(player));
	t4 = (unsigned int)round(stub->GetTalent4(player));
	*/
}
...
/*
int Skill::GetAttack()        { return player->GetAttack((int)round(ratio), (int)round(plus)); }
int Skill::GetMagicattack()   { return player->GetMagicattack((int)round(ratio),(int)round(plus)); }

int Skill::GetCritAttack()        { return player->GetCritAttack((int)round(ratio), (int)round(plus)); }
int Skill::GetCritMagicattack()   { return player->GetCritMagicattack((int)round(ratio),(int)round(plus)); }

void Skill::SetGolddamage(int attack)  { magicdamage[0] = (int)round(attack * (1.0+player->GetIncgold())); }
void Skill::SetWooddamage(int attack)  { magicdamage[1] = (int)round(attack * (1.0+player->GetIncwood())); }
void Skill::SetWaterdamage(int attack) { magicdamage[2] = (int)round(attack * (1.0+player->GetIncwater()));}
void Skill::SetFiredamage(int attack)  { magicdamage[3] = (int)round(attack * (1.0+player->GetIncfire())); }
void Skill::SetEarthdamage(int attack) { magicdamage[4] = (int)round(attack * (1.0+player->GetIncearth()));}
*/
