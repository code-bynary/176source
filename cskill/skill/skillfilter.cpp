...
DEFINE_SUBSTANCE(filter_ImmuneallSK ,filter,CLASS_IMMUNEALLSK);
...
DEFINE_SUBSTANCE(filter_Shenying, filter, CLASS_ATTACKPLUSATR2);
DEFINE_SUBSTANCE(filter_ShenYingjl, filter, CLASS_ADDPRAGA2);
...
DEFINE_SUBSTANCE(filter_SetVersoSombrionv10 ,filter,CLASS_VERSOPALADINNV10);
...
DEFINE_SUBSTANCE(filter_ReduzDanoaDistancia,filter,CLASS_REDUZDANOADISTANCIA); // Reduz dano a dist?ncia
...
DEFINE_SUBSTANCE(filter_SetGlyphCritical6 ,filter,CLASS_GLYPH_CRITICAL6);
...
DEFINE_SUBSTANCE(filter_EnhanceskilldamagePELOTAO2, filter, CLASS_ENHANCESKILLDAMAGEPELOTAO2);
...
DEFINE_SUBSTANCE(filter_DeepenblessWF,filter,CLASS_DEEPENBLESSWF);
DEFINE_SUBSTANCE(filter_IncVigourWando, filter, CLASS_INCVIGOURWANDO);
DEFINE_SUBSTANCE(filter_SetGlyphCritical5, filter, CLASS_GLYPH_CRITICAL5);
DEFINE_SUBSTANCE(filter_FairyformDamage, filter, CLASS_FAIRYFORMDAMAGE);
DEFINE_SUBSTANCE(filter_SetGlyphComboSkill,filter,CLASS_GLYPH_COMBOSKILL);
DEFINE_SUBSTANCE(filter_ShadowformDamage, filter, CLASS_SHADOWFORMDAMAGE);
DEFINE_SUBSTANCE(filter_InchpstealFrenesi,filter,CLASS_INCHPSTEALFRENESI);
DEFINE_SUBSTANCE(filter_SlowWithStacks ,filter,CLASS_SLOW_WITH_STACKS);
DEFINE_SUBSTANCE(filter_skillplusTM, filter, CLASS_SKILLPLUSTM);
DEFINE_SUBSTANCE(filter_Apgencont3 ,filter,CLASS_APGENCONT3);
DEFINE_SUBSTANCE(filter_BeattackedattachstateGlyph,filter,CLASS_BEATTACKEDATTACHSTATEGLYPH);
DEFINE_SUBSTANCE(filter_Immunedirect ,filter,CLASS_IMMUNEDIRECT);
DEFINE_SUBSTANCE(filter_InchurtTM ,filter,CLASS_INCHURTTM);
DEFINE_SUBSTANCE(filter_SealedforTW ,filter,CLASS_SEALEDFORTW);
DEFINE_SUBSTANCE(filter_ImmunedirectforTower ,filter,CLASS_IMMUNEDIRECTFORTOWER);
DEFINE_SUBSTANCE(filter_FullChargeMercSecond ,filter,CLASS_FULLCHARGEMERCSECOND);
DEFINE_SUBSTANCE(filter_ImpairDamageAll, filter, CLASS_IMPAIRDAMAGEALL);
DEFINE_SUBSTANCE(filter_Incfightproperty, filter, CLASS_INCFIGHTPROPERTY);
DEFINE_SUBSTANCE(filter_Freecooldown2 ,filter,CLASS_FREECOOLDOWN2);

// G17/G18
DEFINE_SUBSTANCE(filter_SetIncMoveClear,filter,CLASS_SETINVISIBLEFORCED);
...
DEFINE_SUBSTANCE(filter_InvisibleDelayed,filter,CLASS_INVISIBLEDELAYED);
DEFINE_SUBSTANCE(filter_PragaBless,filter,CLASS_PRAGABLESS);
...
DEFINE_SUBSTANCE(filter_skillplusPAL,filter,CLASS_SKILLPLUSPAL);

// Nova Tw
DEFINE_SUBSTANCE(filter_BuffRecvCataputeDamage ,filter,CLASS_BUFFRECVCATAPUTEDAMAGE);

// Novos filtros Lua > c++
DEFINE_SUBSTANCE(filter_Filter14813 ,filter,CLASS_FILTER14813);
DEFINE_SUBSTANCE(filter_Filter14918 ,filter,CLASS_FILTER14918);

// Reino do Norte
DEFINE_SUBSTANCE(filter_DartCard ,filter,CLASS_FILTERDARTCART);
DEFINE_SUBSTANCE(filter_DartCard2 ,filter,CLASS_FILTERDARTCART2);


// Function to calculate the percentage
double calcularPorcentagem(int hp, int max_hp){
	return static_cast<double>(hp) / static_cast<double>(max_hp) * 100.0;
}
//Function to maintain the percentage
int manterPorcentagem(int new_max_hp, double porcentagem) {
    return static_cast<int>(new_max_hp * porcentagem / 100.0);
}

int arredondar(double valor) {
    return static_cast<int>(std::round(valor));
}

// Function to get the HP after applying the same percentage to a new value
int calcularNovoHP(int new_hp, int max_hp, double porcentagem) {
    return static_cast<int>(porcentagem * new_hp + 0.5);
}

// Fun??o para calcular quanto em HP precisa ser curado para manter a mesma porcentagem
int calcularCuraNecessaria(int hp, int max_hp, int new_hp) {
    double porcentagem_original = calcularPorcentagem(hp, max_hp);
    int novo_max_hp = new_hp > max_hp ? new_hp : max_hp;
    return calcularNovoHP(hp, novo_max_hp, porcentagem_original) - hp;
}

// Function to calculate how much HP needs to be healed to maintain the same percentage
double calcularNovaPorcentagem(int hp, int max_hp, int novo_max_hp) {
    return static_cast<double>(hp) / max_hp;
}

// Function to calculate the new HP after taking damage or reducing max_hp
int calcularHPAposAlteracao(int hp, int max_hp, int novo_max_hp) {
    double porcentagem_original = calcularPorcentagem(hp, max_hp);
    return arredondar(porcentagem_original * novo_max_hp / 100.0);
	//return (porcentagem_original * novo_max_hp / 100.0);
}

int calcularReducaoNecessaria(int hp, int max_hp, int novo_max_hp) {
    double porcentagem_original = calcularPorcentagem(hp, max_hp);
	if(porcentagem_original == 100.0)
	{
		return 0;
	}
	else
	{
		int novo_hp = arredondar(porcentagem_original * novo_max_hp / 100.0);
		//int novo_hp = (porcentagem_original * novo_max_hp / 100.0);
		return hp - novo_hp;
	}
...
void filter_Tigerform::OnAttach()
{
	...
	int max_hp = _parent.GetExtendProp().max_hp;

	double porcentagem = calcularPorcentagem(hp,max_hp);
	...
	int new_max_hp = _parent.GetExtendProp().max_hp;
	int new_hp = manterPorcentagem(new_max_hp, porcentagem);

	if(new_max_hp > max_hp)
		_parent.Heal((new_hp - hp));
...
void filter_Tigerform::OnRelease()
{
	...
	int new_max_hp = _parent.GetExtendProp().max_hp;
	int new_hp = _parent.GetBasicProp().hp;

	int manterPorcentagema = manterPorcentagem(new_max_hp, porcentagem);

	int new_calchp = calcularHPAposAlteracao(hp, max_hp, new_max_hp);

	int reduc_hp = calcularReducaoNecessaria(hp,max_hp,new_max_hp);
/*
	printf("new_hp = %d \n",new_hp);
	printf("new_max_hp = %d \n",new_max_hp);
	printf("Porcentagem = %f \n",porcentagem);
	printf("manterPorcentagem = %d \n",manterPorcentagema);
	printf("new_calchp = %d \n",new_calchp);
	printf("reduc_hp = %d \n",reduc_hp);
	*/
	...
	//_parent.DecHP((hp - manterPorcentagema));
	if(new_max_hp < max_hp)
		_parent.DecHP(reduc_hp);	
...
void filter_Activateskill2::TranslateSendAttack(const XID & target, attack_msg & msg)
{
	...
	if(sid && !(msg._attack_state&AT_STATE_AURA_AND_RETORT))	//Reverse Shock and Aura damage are not processed
	{
		...
		else
		{
			...
			_parent.SetCoolDown(_skillid+COOLINGID_BEGIN,_cooldown* 1000);	
		}
	}
}
...
void filter_Activatereboundskill::AdjustDamage(damage_entry & dmg,const XID & attacker, const attack_msg & msg, float damage_adjust)
{
		...
		// Nova verifica��o de cooldown
		if(!_parent.TestCoolDown(_skillid+COOLINGID_BEGIN))
			return;
...
void filter_RetortNewPaladin::AdjustDamage(damage_entry & dmg,const XID & attacker, const attack_msg & msg, float damage_adjust)
{
	...
	if (_ratio > 0)
	{
		...
		{
			enchant_msg	ret; //create a response parameter
			memset(&ret,0,sizeof(ret));
			...
			_parent.EnterCombatState(); // inflict a retaliatory debuff on the enemy
			_parent.SetReboundState();
			_parent.DelayEnchant(attacker, ret, 10); 
			
			attack_msg * _amsg = (attack_msg *)&msg; //ignore enemy attack
			//_amsg->skill_id = 0;
			_amsg->attached_skill.skill = 0;
			_amsg->attached_skill.level = 0;
			
			--_ratio; // subtract the number of reflected debuffs
		}
	}
	else
	{
