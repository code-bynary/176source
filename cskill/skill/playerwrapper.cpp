...
namespace GNET
{
	...
	int CALC_PHYSIC_DMG(object_interface &object, Skill *skill, int pdmg, int ndmg)
	{
		...
		{
			damage = object.CalcPhysicDamage(pdmg, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
			if (skill->GetPerformerid().IsPlayerClass())
				damage = damage >> 2;
		}
		else
		{
			damage = object.CalcPhysicDamage(ndmg, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
	...
	int CALC_MAGIC_DMG(object_interface &object, Skill *skill, int pdmg, int ndmg)
	{
		...
		if (object.IsPlayerClass())
		{
			damage = object.CalcMagicDamage(mt, pdmg, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
			if (skill->GetPerformerid().IsPlayerClass())
				damage = damage >> 2;
		}
		else
		{
			damage = object.CalcMagicDamage(mt, ndmg, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
			damage = (int)(object.CalcLevelDamagePunish(skill->GetPlayerlevel(), object.GetBasicProp().level) * damage);
			damage = object.CalcPenetrationEnhanceDamage(skill->GetPenetration(), damage);
		}
		return damage;
...
#define LANCA 10056
	...
	void PlayerWrapper::ConvertPlayerBalls(int mode)
	{
		int newvstate = 0, oldvstate = 0, hstate = 0;

		if (skillwrapper && skillwrapper->ConvertAndUpdateBalls(mode, newvstate, oldvstate, hstate))
		{
			if (oldvstate)
				object.DecVisibleState(oldvstate);
			...
			if (!oldvstate)
				object.InsertTeamVisibleState(HSTATE_BLACKWHITE, hstate, -1);
			else
				object.ModifyTeamVisibleState(HSTATE_BLACKWHITE, hstate, -1);
		}
	}
	...
	bool PlayerWrapper::SetPerform(int inform)
	{
		...
		if (skill->GetRange().IsSelf())
		{
			if (skill->GetType() == TYPE_JUMP || success)
			{
				...
				msg.feature = skill->GetGlyphFeature();
				...
				msg.charge = skill->GetConsumedCharge();
				msg.bwballs = skillwrapper->GetBlackWhiteBalls();
				msg.form = object.GetForm();
		...
		else if (skill->GetStub()->DoBless())
		{
			...
			*/
			if(success)
			{
				...
				skill->SetPerformerid(XID(-1,-1));
				skill->SetPlayerlevel(this->GetLevel());
				skill->SetPlayerHighestLevel(this->GetHighestLevel());
				skill->SetMagicDamage(GetMagicattack());
				skill->BlessMe();
				blessmesucc = true;
			}
...
//#ifndef _SKILL_TEST
		...
		if (enable)
		{
			...
			if (intarg == -1)
			{
				//Attack power needs to be recalculated after learning skills
				object.UpdateAttackData();
			}
			return true;
		}
...
//#endif
	...
	bool PlayerWrapper::SetRepel(bool b)
	{
		...
		if (ThrowDice())
		{
			if (object.GetImmuneMask() & (IMMUNEREPEL | IMMUNEALL | IMMUNETWCT))
				immune |= 0x80;
			else
	...
	bool PlayerWrapper::SetRepel5(bool b)
	{
		if (ThrowDice())
		{
			if (object.GetImmuneMask() & (IMMUNEREPEL | IMMUNEALL | IMMUNETWCT))
	...
	bool PlayerWrapper::SetSealedforTW(bool b)
	{
		object.AddFilter(new filter_SealedforTW(object, time));
		return true;
	}
	...
	bool PlayerWrapper::SetFlood(bool)
	{
		...
		damage = object.CalcMagicDamage(2, damage, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
		if (!object.IsPlayerClass())
			damage = object.CalcPenetrationEnhanceDamage(skill->GetPenetration(), damage);
	...
	bool PlayerWrapper::SetThunder(bool)
	{
		...
		int damage = (int)GetAmount();
		damage = object.CalcMagicDamage(0, damage, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
		if (!object.IsPlayerClass())
			damage = object.CalcPenetrationEnhanceDamage(skill->GetPenetration(), damage);
		//	���Ǽ�ǿ����
		//	damage = object.CalcVigourEnhanceDamage(skill->GetVigour(),	object.GetVigour(),	IS_PVP(object,skill),damage);
	...
	bool PlayerWrapper::SetToxic(bool)
	{
		...
		int damage = (int)GetAmount();
		damage = object.CalcMagicDamage(1, damage, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
		if (!object.IsPlayerClass())
			damage = object.CalcPenetrationEnhanceDamage(skill->GetPenetration(), damage);
		//	���Ǽ�ǿ����
		//	damage = object.CalcVigourEnhanceDamage(skill->GetVigour(),	object.GetVigour(),	IS_PVP(object,skill), damage);
	...
	bool PlayerWrapper::SetBurning(bool)
	{
		...
		int damage = (int)GetAmount();
		damage = object.CalcMagicDamage(3, damage, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
		if (!object.IsPlayerClass())
			damage = object.CalcPenetrationEnhanceDamage(skill->GetPenetration(), damage);
		//	���Ǽ�ǿ����
		//	damage = object.CalcVigourEnhanceDamage(skill->GetVigour(),	object.GetVigour(),	IS_PVP(object,skill), damage);
	...
	bool PlayerWrapper::SetFallen(bool)
	{
		...
		int damage = (int)GetAmount();
		damage = object.CalcMagicDamage(4, damage, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
		if (!object.IsPlayerClass())
			damage = object.CalcPenetrationEnhanceDamage(skill->GetPenetration(), damage);
		//	���Ǽ�ǿ����
		//	damage = object.CalcVigourEnhanceDamage(skill->GetVigour(),	object.GetVigour(),	IS_PVP(object,skill), damage);
	...
	bool PlayerWrapper::SetBleeding(bool)
	{
		...
		int damage = (int)GetAmount();
		damage = object.CalcPhysicDamage(damage, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
		if (!object.IsPlayerClass())
			damage = object.CalcPenetrationEnhanceDamage(skill->GetPenetration(), damage);
		//	���Ǽ�ǿ����
		//	damage = object.CalcVigourEnhanceDamage(skill->GetVigour(),	object.GetVigour(),	IS_PVP(object,skill), damage);
	...
	bool PlayerWrapper::SetSlow2(bool b)
	{
		if (ThrowDice())
		{
				int mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
				object.AddFilter(new filter_Slow2(object, time, (int)(ratio * 100), GetValueInt(), mask));
		}
		return true;
	}
...
//#endif
	...
	bool PlayerWrapper::SetFastattack(bool)
	{
		if (ThrowDice())
		{	
			//printf("-------------------- %d fora filtro \n", (int)(ratio * 100 + 0.00001f));
	...
	bool PlayerWrapper::SetScaledechpTower(bool)
	{
		if (ThrowDice())
		{
				if (skill->GetAttackState() & 0x4000 && !object.IsPlayerClass())
	...
	bool PlayerWrapper::SetAp(bool b)
	{
		...
		*/
		if (ThrowDice())
		{
			...
			object.AddFilter(new filter_Apgencont3(object, (int)value, 1));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetIncbow(float inc)
	{
		...
		{
			object.EnhanceScaleDamage((int)(inc * 100));
			//object.EnhanceAttackDegree(8);
			//(object.GetSkillWrapper()).DecPrayTime(12);
			if (intarg == -1)
			{
				object.UpdateAttackData();
				//object.UpdateDefenseData();
		...
		{
			object.ImpairScaleDamage((int)(inc * 100));
			//object.ImpairAttackDegree(8);
			//(object.GetSkillWrapper()).IncPrayTime(12);
		}
		return true;
	}
	...
	bool PlayerWrapper::SetClearTwobuffRandom(bool b)
	{
		if (object.IsFilterExist (FILTER_ANTICLEARBUF2))
		{
			object.RemoveFilter(FILTER_ANTICLEARBUF2);
		...
		if (object.GetImmuneMask() & (IMMUNECLEAR | IMMUNEALL))
			...
			object.ClearTwoRandomSpecFilters(filter::FILTER_MASK_BUFF);
		return true;
		}
	
	}

	bool PlayerWrapper::SetClearOnebuffRandom(bool b)
	{
		if (!object.IsPlayerClass())
			return false;
		...
		if (object.GetImmuneMask() & (IMMUNECLEAR | IMMUNEALL))
			...
			object.ClearOneRandomSpecFilter(filter::FILTER_MASK_BUFF);
		return true;
		}
	...
	bool PlayerWrapper::SetNoregain(bool b)
	{
		if (ThrowDice())
		{
			...
			{
				if(object.GetImmuneMask() & (ALLOWTOWEREFFECT))
				{
	...
	{
		if (ThrowDice())
			object.AddFilter(new filter_Sharpblade(object, (int)(ratio * 100), time, showicon));
		return true;
	}
	...
	bool PlayerWrapper::SetSelfburning(bool)
	{
		...
		int damage = (int)GetAmount();
		damage = object.CalcMagicDamage(3, damage, GetLevel(), skill->GetPlayerHighestLevel());
		//	���Ǽ�ǿ����
		//	damage = object.CalcVigourEnhanceDamage(skill->GetVigour(),	object.GetVigour(),	IS_PVP(object,skill), damage);
	...
	bool PlayerWrapper::SetSealedEPVerdant(bool)
	{
		if (ThrowDice())
		{
			if (object.GetWorldTag() >= 200)
	...
	bool PlayerWrapper::SetFixEPVerdant(bool)
	{
		if (ThrowDice())
		{
			if (object.GetWorldTag() >= 200)
	...
	bool PlayerWrapper::SetDechurt4(bool b)
	{
		if (ThrowDice())
		{
				int mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
				object.AddFilter(new filter_Dechurt4(object, time, ratio, GetValueInt(), mask));
		}
		return true;
	}


	bool PlayerWrapper::SetDechurtEPVerdant(bool)
	{
		if (object.GetWorldTag() >= 200)
		{
			return false;
	...
	bool PlayerWrapper::SetInchurt6(bool)
	{
		if (ratio > 0 && ThrowDice())
		{
			...
			else
			{
				...
				object.AddFilter(new filter_Inchurtfromskill(object, ratio, time, GetAmountInt(),
															 filter_mask, FILTER_INCHURT6, 0, HSTATE_358));
			}
		}
	...
	bool PlayerWrapper::SetIncattack3(bool)
	{
		...
		{	
			int mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Incattack4(object, time, (int)(ratio * 100), GetValueInt(), mask));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetIncmagic3(bool)
	{
		...
		{
			int mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Incmagic3(object, time, (int)(ratio * 100), GetValueInt(), mask));
		}	
		return true;
	}
	...
	bool PlayerWrapper::SetInchpstealFrenesi(bool)
	{
		int cls = GetCls();
		if (cls >= 0 && ((1 << cls) & 0x1ACE))
			return false;
		if (ThrowDice())
		{
			object.AddFilter(new filter_InchpstealFrenesi(object, time, (int)(ratio * 100.f + 0.00001f)));
		}
		return true;
	}
	...
	*/
	...
	{
		if (object.GetImmuneMask() & (IMMUNEPHYSICAL | IMMUNEWEAK | IMMUNEALL))
		{
			immune |= 0x80;
			return true;
		}
		...
		if (object.IsPlayerClass())
		{
			damage = object.CalcPhysicDamage((int)value, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
			if (skill->GetPerformerid().IsPlayerClass())
				damage = damage >> 2;
		}
		else
		{
			damage = object.CalcPhysicDamage((int)amount, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
			damage = (int)(object.CalcLevelDamagePunish(skill->GetPlayerlevel(), object.GetBasicProp().level) * damage);
			damage = object.CalcPenetrationEnhanceDamage(skill->GetPenetration(), damage);
		}
	...
	bool PlayerWrapper::SetWeapondisabled(bool)
	{
		...
		{
			// Custom Fulano
			if(object.IsFilterExist(FILTER_ULTIMATEPALADIN) || object.IsFilterExist(FILTER_REDUCESPEEDDDEBUFF)) return false;
...
*/
	...
	bool PlayerWrapper::SetExchangeposition(bool)
	{
		if (ThrowDice())
		{
			...
			if (object.GetImmuneMask() & (IMMUNETWCT))
				immune |= 0x80;
			else
	...
	bool PlayerWrapper::SetSummonmobs(bool)
	{
		if (ThrowDice())
		{
			...
			{
				object.CreateMinors(param, 5.f,0);
			}
		}
		return true;
	}
	...
	bool PlayerWrapper::SetShadowformDamage(bool b)
	{
		int form = object.GetForm();
		if (form != FORM_NORMAL)
		{
		...
		object.AddFilter(new filter_ShadowformDamage(object, time, (int)(100 * ratio + 0.00001f), value, newvalue));
		return true;
	}
	...
	bool PlayerWrapper::SetFairyformDamage(bool b)
	{
		int form = object.GetForm();
		if (form != FORM_NORMAL)
		{
		...
		object.AddFilter(new filter_FairyformDamage(object, time, (int)(100 * ratio + 0.00001f), (int)(100 * value + 0.00001f), newvalue));
		return true;
	}
	...
	bool PlayerWrapper::SetAddfrosteffect(bool)
	{
		...
		metal_dmg = object.CalcMagicDamage(0, metal_dmg, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
		water_dmg = object.CalcMagicDamage(2, water_dmg, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
		...
		if (object.IsPlayerClass())
		{
			if (skill->GetPerformerid().IsPlayerClass())
			{
				...
				//metal_dmg = (int)metal_dmg;
				//water_dmg = (int)water_dmg;
			}
		}
		else
		{
	...
	bool PlayerWrapper::SetActivefrosteffect(bool)
	{
		...
		if(stacks >= 5)
		{
			...
			{
				object.AddFilter(new filter_GeloAtiradora(object, 4, 1));
	...
	bool PlayerWrapper::SetEnhanceskilldamage3(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_Enhanceskilldamage3(object, time, int(ratio * 100)));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetEnhanceskilldamage4(bool)
	{
		...
		{
			int mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Enhanceskilldamage4(object, time, int(ratio * 100), GetValueInt(), mask));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetDetaindart(bool)
	{
		if (ThrowDice())
		{
			...
			object.AddFilter(new filter_Detaindart(object, time, showicon, is_lighting_effect, GetRatioInt()));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetThunder3(bool)
	{
		...
		thunder_damage = object.CalcMagicDamage(0, thunder_damage, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
		physic_damage = object.CalcPhysicDamage(physic_damage, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
	...
	bool PlayerWrapper::SetToxic3(bool)
	{
		...
		int thunder_damage = GetAmountInt();
		int physic_damage = GetValueInt();
		thunder_damage = object.CalcMagicDamage(1, thunder_damage, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
		physic_damage = object.CalcPhysicDamage(physic_damage, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
		int damage = thunder_damage + physic_damage;
		if (!object.IsPlayerClass())
		{
	...
	bool PlayerWrapper::SetFlood3(bool)
	{
		...
		int thunder_damage = GetAmountInt();
		int physic_damage = GetValueInt();
		thunder_damage = object.CalcMagicDamage(2, thunder_damage, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
		physic_damage = object.CalcPhysicDamage(physic_damage, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
		int damage = thunder_damage + physic_damage;
		if (!object.IsPlayerClass())
		{
	...
	bool PlayerWrapper::SetBurning3(bool)
	{
		...
		int thunder_damage = GetAmountInt();
		int physic_damage = GetValueInt();
		thunder_damage = object.CalcMagicDamage(3, thunder_damage, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
		physic_damage = object.CalcPhysicDamage(physic_damage, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
		int damage = thunder_damage + physic_damage;
		if (!object.IsPlayerClass())
		{
	...
	bool PlayerWrapper::SetFallen3(bool)
	{
		...
		int thunder_damage = GetAmountInt();
		int physic_damage = GetValueInt();
		thunder_damage = object.CalcMagicDamage(4, thunder_damage, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
		physic_damage = object.CalcPhysicDamage(physic_damage, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
		int damage = thunder_damage + physic_damage;
		if (!object.IsPlayerClass())
		{
	...
	bool PlayerWrapper::SetRepelAtr(bool b)
	{
		if (!object.IsPlayerClass() && !object.IsInTeam()) {
			...
			if (object.GetImmuneMask() & (IMMUNEREPEL | IMMUNEALL | IMMUNETWCT))
				...
				object.KnockBack(skill->GetPerformerid(), skill->GetPerformerpos(), value, (int)amount, time);
				object.SetInvincibleFilter(true, 2, true);
				object.AddFilter(new filter_Icon(object, 2, HSTATE_INVINCIBLE));
			}
	...
	{
		object.ClearSpecFilter(filter::FILTER_MASK_DEBUFF | filter::FILTER_MASK_BLACK_STUN );
	...
	bool PlayerWrapper::SetRemoveDelayed(bool)
	{
		if(amount < 1) 
		{
			return false;
		...
		{
			object.AddFilter(new filter_SetRemoveNew(object, time));
		}
	...
	bool PlayerWrapper::SetLoaddmg4master(bool b)
	{
		...
		//{
			int mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
			object.AddFilter(new filter_Loaddmg4master(object, time, ratio, (int)value, (int)probability, mask ));
		//}
		return true;
	}
	//ULTRT
	...
	bool PlayerWrapper::SetSkillcooldown3(bool)
	{
		if (ThrowDice())
		{
			if(time < 1)
			...
			object.ResetAllCoolDown(time, skill->GetId() + COOLINGID_BEGIN);
		}
		return true;
	}
	...
	bool PlayerWrapper::SetShenYing(bool)
	{
		if(object.GetCurTargetID() == object.GetSelfID()) return false;
		...
		if (ThrowDice())
		{
			...
			if(!object.QueryObject(object.GetCurTargetID(), pos, body_size)) return false;

			object.AddFilter(new filter_Shenying(object, object.GetCurTargetID(), ratio, value, pos, time));
		}
		return true;
	}


	bool PlayerWrapper::SetShenYingjl(bool)
	{
		if(skill->GetPerformerid () == object.GetSelfID()) return false;
		...
		if (ThrowDice())
		{
			if (object.GetImmuneMask() & IMMUNEALL) {
				...
				object.AddFilter(new filter_ShenYingjl(object, time));
			}						
		}
		return true;
	}
	...
	bool PlayerWrapper::SetAddPraga(bool)
	{
		if (ThrowDice())
		{
			if (object.GetImmuneMask() & (IMMUNEREPEL | IMMUNEALL | IMMUNETWCT)) {
				...
				object.AddFilter(new filter_AddPraga(object, time, skill->GetPerformerid()));
			}
	...
	bool PlayerWrapper::SetPragaBless(bool)
	{
		if (ThrowDice())
		{
			//if (skill->GetPerformerid() != object.GetSelfID())
				const XID &cur_target = object.GetCurTargetID();
				object.AddFilter(new filter_PragaBless(object, time, cur_target));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetVersoSombrionv10(bool) // Mesmo Filtro 
	{
		if (ThrowDice())
			object.AddFilter(new filter_SetVersoSombrionv10(object, (int)(ratio * 100), (int)(amount * 100.f + 0.00001f), time));
		return true;
	}
	...
	// asn combo skill	
	unsigned int PlayerWrapper::GetChargeMerc()
	{
		//if(object.GetCountChargeMerc() > 0)
		//{
		//	printf("PlayerWrapper::GetChargeMerc: val = %d \n", object.GetCountChargeMerc());
			return object.GetCountChargeMerc();
		//}	
		//return 0;
	}

	unsigned int PlayerWrapper::CountChargeMerc()
	{
		//if(object.GetCountChargeMerc() > 0)
		//{
		//	printf("PlayerWrapper::CountChargeMerc: val = %d \n", object.GetCountChargeMerc());
			return object.GetCountChargeMerc();
		//}	
		//return 0;
	}
	
	void PlayerWrapper::GainChargeMerc(unsigned int val )
	{
		if(val > 0)
		{
		//	printf("PlayerWrapper::GainChargeMerc: val = %d \n", val);
	...
	void PlayerWrapper::SpendChargeMerc(unsigned int val )
	{
		
		if(val > 0)
		{
		//	printf("PlayerWrapper::SpendChargeMerc: val = %d \n", val);
			if(val > 5) val = 5;
			object.ImpairChargeMerc( val );
		}	
	}

	bool PlayerWrapper::SpendChargeMercWithProb(bool)
	{
		if (ThrowDice())
		{
			if(value > 0)
			{
				object.ImpairChargeMerc(value);
			}
		}
		return true;
	}

	void PlayerWrapper::SpendChargeMerc2(unsigned int val )
	{
		if(val > 0)
		{
		//	printf("PlayerWrapper::SpendChargeMerc2: val = %d \n", val);
			object.ImpairChargeElseMerc(skill->GetPerformerid().id, val);
		}
	}
	// asn combo skill end
	...
	bool PlayerWrapper::SetFreecooldown2(bool)
	{
		if (ThrowDice())
		{
			/*if (object.GetImmuneMask() & (IMMUNECURSED | IMMUNEALL))
				...
				time = 1 + GetBufflevel()->GetValue (4390);
				object.AddFilter(new filter_Freecooldown2(object, time, (int)value, (int)ratio, (int)amount, (int)newvalue, (int)newvalue2));
		}
		return true;
	}
...
//++++++++++++172++++++++++++++//
	...
	bool PlayerWrapper::SetMonkeyformUltimate(bool b)
	{
		if(ThrowDice())
		{
			object.AddFilter(new filter_MonkeyformUltimate(object, time));		
		}
		return true;
	}
	...
	{
		if (enable)
			object.SetMonkyPassiveReduce(inc, true);
		else
			object.SetMonkyPassiveReduce(inc, false);
	...
	bool PlayerWrapper::SetIncAtkFToDefense(bool)
	{
		if (ThrowDice())
		{
			int damage = object.GenerateDamageWithoutRune();

			damage = (int)(damage * 0.8);

			object.AddFilter(new filter_IncAtkFToDefense(object, ratio, damage, time));
		}
	...
	bool PlayerWrapper::SetReduceDefensePassive(float inc) // Redução de Defesa passiva nova
	{
		if (enable)
		{
			object.IncMaxDefense(inc);
			return true;
		}
		else
		{
			object.DecMaxDefense(inc);
			return true;
		}		
		return false;
	}
...
#define WEAPONCLASS_BOXING_NEW		50177
	...
	bool PlayerWrapper::GetUltimateEnabled()
	{
		if(skillwrapper && skill)
			return skillwrapper->GetUltimateEnabled(); 
		else 
			return false;
	}
	...
	bool PlayerWrapper::SetGlyphCritical6(bool b) // Atiradora
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_SetGlyphCritical6(object, time, GetRatioInt()));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetSummonpet3(bool)
	{
		...
		object.SummonPet3(GetValueInt(), GetRatioInt(), time, cur_target, skill->GetForceattack()); //value��pet_egg_id
		return true;
	}
	...
	bool PlayerWrapper::SetEnhanceskilldamagePELOTAO2(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_EnhanceskilldamagePELOTAO2(object, time, int(ratio * 100)));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetShieldDamageReduce(bool B)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_SetShieldDamageReduce(object, value, (int)amount, time));
		}
		return true;
	}
	...
	// Aumenta o dano enquanto ele se move
	/*bool PlayerWrapper::SetApplyDamageforRange(bool b)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_SetApplyDamageforRange(object, time, (int)(100 * ratio + 0.00001f), object.GetPos()));
		}
		return true;
	}*/
	...
	bool PlayerWrapper::SetShieldTransferDebuffBarbian(bool)
	{
		if (ThrowDice())
		{
			...
			if (object.IsFilterExist(FILTER_IMMUNETRANSFWB) || (object.GetImmuneMask() & (IMMUNETRANSFWB | IMMUNEALL)))
				{
					immune |= 0x80;
					return false;
				}
			...
			//object.AddFilter(new filter_ShieldTransferDebuffBarbian(object, time, skill->GetPerformerid()));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetContinuousDamageWood(bool) // Madeira
	{
		...
		int damage = object.CalcMagicDamage(ELE_WOOD, thunder_damage, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
	...
	bool PlayerWrapper::SetFilterDebuffSK4Inc(bool) // Sangria
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_DebuffGlyphSK4(object, time));
		}
		return true;
	}

	bool PlayerWrapper::SetFilterDebuffSK4Dec(bool)
	{
		if (ThrowDice())
		{
			if(object.IsFilterExist(FILTER_DEBUFFGLYPHSK4))
			{
				object.RemoveFilter(FILTER_DEBUFFGLYPHSK4);
			}
		}
		return true;
	}
	...
	bool PlayerWrapper::SetForceCriticalAttack2(bool)
	{
		int newtime = 7; 
		object.AddFilter(new filter_SetGlyphCritical5(object, newtime));
		return true;
	}
	...
	bool PlayerWrapper::SetPhysMagReducwithstackWB(bool)
	{
		if (ThrowDice())
		{
			...
			object.AddFilter(new filter_PhysMagReducwithstackWB(object, time, (int)(ratio), (int)(value), (int)amount));	
		}
		return true;
	}
	...
	bool PlayerWrapper::SetNewStun(bool b)
	{
		if (ThrowDice())
		{
			...
			object.AddFilter(new filter_NewStun(object, time));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetElementDamage(bool)
	{
		...
		dmg = object.CalcMagicDamage(element, dmg, skill->GetPlayerlevel(), skill->GetPlayerHighestLevel());
	...
	bool PlayerWrapper::SetDeepenblessWF(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_DeepenblessWF(object, time, int(value)));
		}
		return true;
	}

	bool PlayerWrapper::SetIncVigourWando(bool)
	{
		if (ThrowDice())
			object.AddFilter(new filter_IncVigourWando(object, time, int(value)));
		return true;
	}

	bool PlayerWrapper::SetGlyphComboSkill(bool b)
	{
		if(ThrowDice())
		{
			object.AddFilter(new filter_SetGlyphComboSkill(object, time, (int)value, int(amount)));
		}
		return true;
	}

	bool PlayerWrapper::SetGlyphComboSkillDec(bool b)
	{
		if (!object.IsFilterExist(FILTER_GLYPH_COMBOSKILL)) return false;

		object.RemoveFilter(FILTER_GLYPH_COMBOSKILL);
		return true;
	}

	bool PlayerWrapper::SetSlowWithStacks(bool b)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_SlowWithStacks(object, (int)(ratio * 100), time, value));
		}
		return true;
	}

	bool PlayerWrapper::SetRemoveSlowWithStacks(bool b)
	{
		if(!object.IsFilterExist(FILTER_SLOW_WITH_STACKS)) return false;

		object.RemoveFilter(FILTER_SLOW_WITH_STACKS);

		return true;
	}

	bool PlayerWrapper::SetskillplusTM(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_skillplusTM(object, (int)(value * 100), time));
		}
		return true;
	}

	bool PlayerWrapper::SetBeattackattachstateGotadeSangue(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_BeattackedattachstateGlyph(object, time, GetRatioInt(), GetValueInt(), GetAmountInt(), true, HSTATE_357, FILTER_BEATTACKEDATTACHSTATEGOTA));
		}
		return true;
	}
	bool PlayerWrapper::SetBeattackattachstateEspiritoArruinado(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_BeattackedattachstateGlyph(object, time, GetRatioInt(), GetValueInt(), GetAmountInt(), true, HSTATE_356, FILTER_BEATTACKEDATTACHSTATEESPIRITO));
		}
		return true;
	}
	bool PlayerWrapper::SetBeattackattachstateDesarmado(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_BeattackedattachstateGlyph(object, time, GetRatioInt(), GetValueInt(), GetAmountInt(), true, HSTATE_355, FILTER_BEATTACKEDATTACHSTATEDESARMADO));
		}
		return true;
	}
	bool PlayerWrapper::SetBeattackattachstateOlhodoCeu(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_BeattackedattachstateGlyph(object, time, GetRatioInt(), GetValueInt(), GetAmountInt(), true, HSTATE_354, FILTER_BEATTACKEDATTACHSTATEOLHODOCEU));
		}
		return true;
	}

	bool PlayerWrapper::SetResetCooldownFeature(bool)
	{
		if (ThrowDice())
		{
			skillwrapper->ResetSkillFeatureCooldown(object, (int)value);
		}
		return true;
	}

	bool PlayerWrapper::SetResetCooldownFeatureWith3SkillsID(bool)
	{
		if (ThrowDice())
		{
			skillwrapper->ResetSpecificSkillCooldowns(object, (int)value, (int)ratio, (int)amount, (int)newvalue);
		}
	}

	bool PlayerWrapper::SetResetCooldownFeatureWith4SkillsID(bool)
	{
		if (ThrowDice())
		{
			skillwrapper->ResetSpecificSkillCooldowns(object, (int)value, (int)ratio, (int)amount, (int)newvalue, (int)newvalue2);
		}
	}

	bool PlayerWrapper::SetDecCooldownFeatureWith3SkillsID(bool)
	{
		if (ThrowDice())
		{											   /*feature*/  /*seconds*/ /*exclude*/     /*id1*/	       /*id2*/		   /*id3*/
			skillwrapper->ReduceSkillCooldowns(object, (int)value, (int)ratio, (int)amount, (int)newvalue, (int)newvalue2, (int)newvalue3);
		}
	}

	bool PlayerWrapper::SetDecCooldownFeatureWith4SkillsID(bool)
	{
		if (ThrowDice())
		{											   /*feature*/  /*seconds*/ /*exclude*/     /*id1*/	       /*id2*/		   /*id3*/		   /*id4*/
			skillwrapper->ReduceSkillCooldowns(object, (int)value, (int)ratio, (int)amount, (int)newvalue, (int)newvalue2, (int)newvalue3, (int)newvalue4);
		}
	}

	bool PlayerWrapper::SetDecCooldownFeatureWith5SkillsID(bool)
	{
		if (ThrowDice())
		{											   /*feature*/  /*seconds*/ /*exclude*/     /*id1*/	       /*id2*/		   /*id3*/		   /*id4*/
			skillwrapper->ReduceSkillCooldowns(object, (int)value, (int)ratio, (int)amount, (int)newvalue, (int)newvalue2, (int)newvalue3, (int)newvalue4, (int)newvalue5);
		}
	}

	bool PlayerWrapper::SetImmunneDirect(bool b)
	{
		if (ThrowDice())
		{
			if (object.IsFilterExist(FILTER_IMMUNEDIRECT))
			{
				object.RemoveFilter(FILTER_IMMUNEDIRECT);
			}

			object.AddFilter(new filter_Immunedirect(object, time));
		}
		return true;
	}

	bool PlayerWrapper::SetInchurtTM(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_InchurtTM(object, ratio, time));
		}
		return true;
	}

	bool PlayerWrapper::SetImmunneDirectforTower(bool b)
	{
		if (ThrowDice())
		{

			object.AddFilter(new filter_ImmunedirectforTower(object, time));
		}
		return true;
	}

	bool PlayerWrapper::SetFullChargeMercSecond(bool)
	{
		if (ThrowDice())
		{
			object.EnhanceChargeMerc(5);
			object.AddFilter(new filter_FullChargeMercSecond(object, time));
		}
	}

	bool PlayerWrapper::SetDirecthurtForSiege(bool)
	{
		if (!ThrowDice())
			return false;
	...
	bool PlayerWrapper::SetImmuneallSK(bool b)
	{
		if (ThrowDice())
		{
			object.ClearSpecFilter(filter::FILTER_MASK_DEBUFF);
			object.AddFilter(new filter_ImmuneallSK(object, time));
		}
		return true;
	}

	bool PlayerWrapper::SetImpairDamageAll(bool b)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_ImpairDamageAll(object, time, ratio));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetIncfightproperty(bool)
	{
		...
		int mask = (amount > 1.0f ? 0 : (amount > 0.5f ? filter::FILTER_MASK_BUFF : filter::FILTER_MASK_DEBUFF));
		object.AddFilter(new filter_Incfightproperty(object, time, (int)(ratio * 100), (int)(value * 100), (int)(probability * 100), mask));
		return true;
	}

	bool PlayerWrapper::SetBleedingMordidaViolenta(bool)
	{
		if (!ThrowDice())
			return false;
		if (object.GetImmuneMask() & (IMMUNEBLOODING | IMMUNEALL)) //lgc
	...
	bool PlayerWrapper::SetInvisibleDelayed(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_InvisibleDelayed(object, time));
		}
		return true;
	}
	...
	{
		object.AddFilter(new filter_SoulbeatbackWf(object, time, (int)value, (int)probability, (int)ratio, 0));
		return true;
	}

	bool PlayerWrapper::SetSoulbeatbackPaladin(bool)
	{		
		object.AddFilter(new filter_SoulbeatbackPaladin(object, time, (int)value, (int)probability, (int)ratio, 0, GetNewValueInt() /*Max_skills*/));
		return true;
	}

	bool PlayerWrapper::SetskillplusPAL(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_skillplusPAL(object, (int)(value * 100), time));
		}
		return true;
	}
	...
	bool PlayerWrapper::SetNewShapeAndSkills(bool)
	{
		...
		if(GetRatioInt() == 1)
		{
			object.AddFilter(new filter_NewShapeAndSkills(object, GetValueInt(), GetAmountInt(), GetNewValueInt(), GetNewValue2Int(), GetNewValue3Int()));
	...
	bool PlayerWrapper::SetAddSpeedup(float inc)
	{
	...
	bool PlayerWrapper::SetIncMoveClear(bool)
	{
		if (ThrowDice())
		{
			if(enable)
				object.AddFilter(new filter_SetIncMoveClear(object, time, skill->GetPerformerpos ()));
			else
	...
	bool PlayerWrapper::SetForceTransform(bool)
	{
		...
		{
			if (enable)
			object.AddFilter(new filter_SetForceTransform(object, time, GetValueInt (), skill->GetPerformerpos ()));
			else 
	...
	bool PlayerWrapper::SetRenameCharacter(bool)
	{
		if (ThrowDice())
		{
			if (enable)
				object.SendClientQilingRename(2, object.GetSelfID().id, true, true, false, false);
			else 
				object.SendClientQilingRename(2, object.GetSelfID().id, false, false, false, false);
		}
		return true;
	}
	...
	bool PlayerWrapper::SetAdjustDamageSendRecv(bool)
	{
		...
		{
			if(enable)
				object.AddFilter(new filter_SetAdjustDamageSendRecv(object, time, ratio));
			else
	...
	bool PlayerWrapper::SetAdjustDamageSendRecv2(bool)
	{
		...
		{
			if(enable)
			object.AddFilter(new filter_SetAdjustDamageSendRecv2(object, time, ratio));
			else
	...
	bool PlayerWrapper::SetIncDamageReducePerc(bool)
	{
		...
		{
			if(enable)
				object.AddFilter(new filter_SetIncDamageReducePerc(object, time, ratio, value, amount));
			else
	...
	bool PlayerWrapper::SetIncImmuneNegativeEffect(bool)
	{
		...
		{
			/*if(enable)
			object.AddFilter(new filter_SetIncImmuneNegativeEffect(object, time));
			else
...
//#endif
	...
	char PlayerWrapper::GetGenesisLevelEnabled()
	{
		bool res = false;
		
		if(skillwrapper && skill)
		{
			res = skillwrapper->GetGenesisLevelEnabled();
		}
		return res;
	}

	// Nova Tw
	bool PlayerWrapper::BuffRecvCataputeDamage(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_BuffRecvCataputeDamage(object, time));
		}
		return true;
	}

	// Novos filtros Lua > c++
	bool PlayerWrapper::SetFilter14813(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_Filter14813(object, time));
		}
		return true;
	}

	bool PlayerWrapper::SetFilter14918(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_Filter14918(object, time));
		}
		return true;
	}

	bool PlayerWrapper::SendDuskLongJump(bool)
	{
		if (ThrowDice())
		{
			A3DVECTOR pos;
			...
			object.LongJump(pos,tag_map);
		}
		return true;
	}

	bool PlayerWrapper::SetDartCard(bool)
	{
		if (ThrowDice())
		{
			object.AddFilter(new filter_DartCard(object, time, GetValueInt () /*level*/, GetAmountInt() /*mobid*/));
		}
		return true;
	}
	bool PlayerWrapper::SetDartCard2(bool)
	{
		if (ThrowDice())
		{
			if(GetValueInt () == 1)
			{
				object.AddFilter(new filter_DartCard2(object, time, GetAmountInt() /*mobid*/));
			} else 
			{
				if(object.IsFilterExist(FILTER_FILTERDARTCART2))
				{
					object.RemoveFilter(FILTER_FILTERDARTCART2);
				}
			}
		}
		return true;
