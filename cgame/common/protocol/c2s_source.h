...
// -- cgame recv new 155 - 162 -- //
		...
		struct capture_item_manager
		{
			...
			unsigned int size; //15 ~ 23
			unsigned int size2; //1 ~ 9
			VALUES values[1];
			char name[]; //request_history ~ GET_BOTTLE_INFO_HISTORY		
		};
...
// -- cgame recv new 162 - 172 -- //
		...
		struct cgame_send_275
		{
			cmd_header header;
		};

		struct cgame_send_276
		{
			cmd_header header;
		};

		struct activity_new_manager
		{
			enum
			{
				LOGIN_REWARD = 0,
		...
		struct cgame_send_278
		{
			cmd_header header;
		};

		struct cgame_send_279
		{
			cmd_header header;
		};

		struct cgame_send_280
		{
			cmd_header header;
		};

		struct cgame_send_281
		{
			cmd_header header;
		};
	...
	namespace CMD // main struct 
	{
		...
		struct new_arena_get_reward
		{
			cmd_header header;
		};

		struct enter_role_new
		{
			cmd_header header;
			int roleid;
		};
