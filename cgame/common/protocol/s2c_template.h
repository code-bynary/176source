	...
	namespace CMD
	{
		...
		struct Make<INFO::self_info_1>
		{
			...
			static WRAPPER & From(WRAPPER & wrapper,gplayer *pPlayer,int exp,int sp)
			{
				...
				<< (unsigned char)pPlayer->dir << (unsigned char) pPlayer->sec_level << state << pPlayer->object_state2 << pPlayer->object_state3; /*176+*/				
		...
		struct Make<INFO::player_info_1>
		{
			...
			static bool From(WRAPPER & wrapper,gplayer * pObject)
			{
				...
				<< (unsigned char)pObject->dir << (unsigned char) pObject->sec_level << state << pObject->object_state2 << pObject->object_state3; /*176+*/		
		...
		{
			template <typename WRAPPER>
			static WRAPPER & From(WRAPPER & wrapper,gnpc * pObject, bool EnabledRoleid)
			{
				unsigned int state = MakeObjectState(pObject) | pObject->object_state;
				...
				if(pObject->leader_roleid > 0 && EnabledRoleid) 
				{
					wrapper << pObject->leader_roleid; /*176+*/
				}
			...
			static WRAPPER & From(WRAPPER & wrapper,gnpc * pObject,const A3DVECTOR &newpos)
			{
				...
				wrapper << pObject->ID.id << pObject->tid << pObject->vis_tid << newpos
					<< (unsigned short)pObject->crc << (unsigned char)pObject->dir << state << pObject->object_state2 << pObject->object_state2;
		...
		struct Make<CMD::npc_enter_world>
		{
			...
			static WRAPPER & From(WRAPPER & wrapper,gnpc * pNPC)
			{
				...
				return Make<INFO::npc_info>::From(wrapper,pNPC,true);
			}
		};
		...
		{
			template <typename WRAPPER>
			inline static WRAPPER & From(WRAPPER & wrapper,unsigned int status_point,const struct extend_prop & prop, const extend_prop & base_prop, int attack_degree, int defend_degree, int crit_rate, int crit_damage_bonus, int invisible_degree, int anti_invisible_degree, int penetration, int resilience, int vigour, int anti_def_degree, int anti_resist_degree, 
			int damage_reduce, int*magic_reduce, int prayspeed, int crit_damage_reduce, int peak_grade, int soul_power, int hp, float max_reduce)
			{
				Make<single_data_header>::From(wrapper,SELF_GET_EXT_PROPERTY);
				wrapper << status_point << attack_degree << defend_degree << crit_rate << crit_damage_bonus << invisible_degree << anti_invisible_degree << penetration << resilience << vigour << anti_def_degree << anti_resist_degree << prop << max_reduce << base_prop <<
					max_reduce << damage_reduce << magic_reduce[0] << magic_reduce[1] << magic_reduce[2] << magic_reduce[3] << magic_reduce[4] << prayspeed << crit_damage_reduce << peak_grade << soul_power << hp;				
				return wrapper;
			}
		};
		...
		struct Make<CMD::send_equipment_info>
		{
			...
			inline static WRAPPER & From(WRAPPER & wrapper,gplayer* pPlayer,uint64_t mask,OCTETS &data, const void * buf, unsigned int size)
			{
		...
		template <>
		struct Make<CMD::new_arena_reward_info>
		{
			template <typename WRAPPER>
			inline static WRAPPER & From(WRAPPER & wrapper,int reward_count)
			{
				Make<single_data_header>::From(wrapper,NEW_ARENA_TEAM_REWARD);
				return wrapper << reward_count;
			}
		};

		template <>
		struct Make<CMD::new_arena_get_open>
		{
			template <typename WRAPPER>
			inline static WRAPPER & From(WRAPPER & wrapper,char open)
			{
				Make<single_data_header>::From(wrapper,NEW_ARENA_GET_OPEN);
				return wrapper << open;
			}
		};
		...
		template <>
		struct Make<CMD::new_arena_players_alive>
		{
			template <typename WRAPPER>
			inline static WRAPPER & From(WRAPPER & wrapper,int red_alive_count, int max_red_alive_count, int blue_alive_count, int max_blue_alive_count)
			{
				Make<single_data_header>::From(wrapper,NEW_ARENA_PLAYERS_ALIVE);
				return wrapper << red_alive_count << max_red_alive_count << blue_alive_count << max_blue_alive_count;
			}			
		};

		template <>
		struct Make<CMD::new_arena_relogin_request>
		{
			template <typename WRAPPER>
			inline static WRAPPER & From(WRAPPER & wrapper)
			{
				return Make<single_data_header>::From(wrapper,NEW_ARENA_RELOGIN_REQUEST);
			}			
		};
...
// -- cgame recv new 155 - 162 -- //
		...
		template<>
		struct Make<CMD::capture_item_info>
		{
			template<typename WRAPPER>
			inline static WRAPPER & From( WRAPPER & wrapper , const void * buf, unsigned int size )
			{
				Make<single_data_header>::From(wrapper,CAPTURE_ITEM_INFO);
				wrapper << size;
				if(size) wrapper.push_back(buf,size);
				return wrapper;
			}
...
// -- cgame recv new 162 - 172 -- //
		...
		/*165+ Reino do Norte*/
		template <>
		struct Make<CMD::dart_car_info>
		{
			template <typename WRAPPER>
			inline static WRAPPER& From(WRAPPER& wrapper, unsigned int roleid, unsigned int level, unsigned int mobid, unsigned int mode)
			{
				Make<single_data_header>::From(wrapper, DART_CAR_INFO);
				return wrapper << roleid << level << mobid << mode;
			}
		};

		template <>
		struct Make<CMD::drunk_status>
		{
			template <typename WRAPPER>
			inline static WRAPPER& From(WRAPPER& wrapper, unsigned int time, int mode)
			{
				Make<single_data_header>::From(wrapper, DRUNK_STATUS);
				return wrapper << time << mode;
			}
		};
		...
		template<>
		struct Make<CMD::association_player_info>
		{
			template <typename WRAPPER>
			inline static WRAPPER& From(WRAPPER& wrapper, unsigned int size, const void * info)
			{
				Make<single_data_header>::From(wrapper, ASSOCIATION_PLAYER_INFO);
				wrapper.push_back(info, size);	
				return wrapper;
			}
		};
		...
		template<>
		struct Make<CMD::association_pvp_mode>
		{
			template <typename WRAPPER>
			inline static WRAPPER& From(WRAPPER& wrapper, char mode)
			{
				Make<single_data_header>::From(wrapper, ASSOCIATION_PVP_MODE);
				return wrapper << mode;
			}
		};

		template<>
		struct Make<CMD::association_player_level_up>
		{
			template <typename WRAPPER>
			inline static WRAPPER& From(WRAPPER& wrapper, char old_level, char new_level)
			{
				Make<single_data_header>::From(wrapper, ASSOCIATION_PLAYER_LEVEL_UP);
				return wrapper << old_level << new_level;
			}
		};
		...
		template <>
		struct Make<CMD::person_interface_notify>
		{
			template <typename WRAPPER>
			inline static WRAPPER& From(WRAPPER& wrapper, int key_type, int key_id, int key_value)
			{
				Make<single_data_header>::From(wrapper, PERSON_INTERFACE_NOTIFY);
				return wrapper << key_type << key_id << key_value;
			}
		...
		{
			template <typename WRAPPER>
			inline static WRAPPER& From(WRAPPER& wrapper, unsigned int size, const void *buf)
			{
		...
		template <>
		struct Make<CMD::activity_event_buy_bar>
		{
			template <typename WRAPPER>
			inline static WRAPPER& From(WRAPPER& wrapper, unsigned int time_end, unsigned int silver_used, int awards_pos)
			{
				Make<single_data_header>::From(wrapper, ACTIVITY_EVENT_BUY_BAR);
				return wrapper << time_end << silver_used << awards_pos;
			}
		};
