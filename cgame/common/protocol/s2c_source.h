...
namespace S2C
{
	...
	namespace CMD
	{
		...
		struct self_get_property
		{
			...
			//extend_prop prop;
			float max_reduce;
			//extend_prop base_prop;
			float max_reduce2;
			int damage_reduce;
			int magic_damage_reduce[5];
			int prayspeed;
			int crit_damage_reduce;
			int peak_grade;
			int soul_power;
			int hp;
		};
		...
		struct capture_item_info
		{
			...
			single_data_header header;
			unsigned int mode; //6 ~ 7
			unsigned int size; //0
			unsigned int size2; //1 ~ 9
			VALUES values[];
		...
		{
			single_data_header header;
			//size = sizeof(single_data_header) (unsigned int)-2;		 
		};

		struct cgame_recv_526
		{
			single_data_header header;
			//size = sizeof(single_data_header) size == 30 * *((_DWORD *)a3 + 1) + 8;
		};

		struct dart_car_info
		{
			single_data_header header;
			int roleid;
			int level;
			int mobid;
			int mode; //0 - ativo | 1 - inativo
		};
		...
		{
			single_data_header header;
			//size = sizeof(single_data_header) ERRORR = 0;
		};
		...
		struct association_pvp_mode
		{
			single_data_header header;
			unsigned char mode; // 0 = normal, 1 = vermelho, 2 = azul
		};

		struct association_player_level_up
		{
			single_data_header header;
			unsigned char old_level; //0
			unsigned char new_level; //1
		};

		struct association_task_info
		{
			single_data_header header;
			unsigned int count;
		...
		struct person_interface_notify
		{
			single_data_header header;
			int key_type; //1
			int key_id;
		...
		struct cgame_recv_586
		{
			single_data_header header;
			int reserved;
			unsigned int tid; //69085
			unsigned int id; //2215731924
			A3DVECTOR pos; 
			char reserved1; //28
		};
		...
		struct activity_event_shop
		{
			struct ITEMS
			{
				int limit_mode; // 1 = day || 4 = limited
				int item_id;
				int limit_count;
			};			
		...
		struct cgame_recv_595
		{
			single_data_header header;
			//size = sizeof(single_data_header) + ; 	
		};

		struct cgame_recv_596
		{
			single_data_header header;
			//size = sizeof(single_data_header) + ; 	
		};

		struct cgame_recv_597
		{
			single_data_header header;
			//size = sizeof(single_data_header) + ; 	
		};

		struct activity_event_buy_bar
		{	
			single_data_header header;
			unsigned int time_end;
			unsigned int silver_used; 
			int awards_pos;
		};

		struct cgame_recv_599
		{
			single_data_header header;
			short reserved;
			short reserved2;	
		...
		struct cgame_recv_600
		{
			single_data_header header;
			//size = sizeof(single_data_header) + ; 	
		};

		struct cgame_recv_601
		{
			single_data_header header;
			//size = sizeof(single_data_header) + ; 	
		};

		struct cgame_recv_602
		{
			single_data_header header;
			int roleid; // 2720771 ??
			int roleid2; // 2720771 ??
		...
		struct cgame_recv_603
		{
			single_data_header header;
			int caster_id;	// roleid
			int target_id; // id servidor	 
			int reserved; // 4256	 	
			int skill_id; // 6696
		};

		struct cgame_recv_604
		{
			single_data_header header;
			//size = sizeof(single_data_header) + ; 	
		};
	...
	namespace CMD
	{
		...
		struct new_arena_reward_info
		{
			single_data_header header;
			int reward_count;
		};		

		struct new_arena_get_open
		{
			single_data_header header;
			char open;
		};
		...
		struct new_arena_players_alive
		{
			single_data_header header;
			int red_alive_count;
			int max_red_alive_count;
			int blue_alive_count;
			int max_blue_alive_count;
		};

		struct new_arena_relogin_request
		{
			single_data_header header;
		};
