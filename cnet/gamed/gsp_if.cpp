...
// arena queue
void EC_SendArenaEnterQueue(int leader_id, int cls, const void * name, unsigned int name_size, int * members, unsigned int count, int team_id, int mode_type, int team_score)
{
	EC_ArenaEnterQueue proto(leader_id, cls);
	proto.name.replace(name, name_size);
	for (unsigned int i = 0; i < count && i < 6; i++)
	{
		proto.members.push_back(members[i]);
	}
	proto.team_id = team_id;
	proto.team_type = mode_type;
	proto.team_score = team_score;
	GProviderClient::DispatchProtocol(0, proto);
}
...
void EC_SendArenaBattleEnd(int arena_id, int world_tag, int result, int red_alive, int blue_alive, int red_damage, int blue_damage)
{
